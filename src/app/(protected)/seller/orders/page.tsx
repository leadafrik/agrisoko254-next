...
export default function SellerOrdersPage() {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      setOrders(response?.data ?? response?.orders ?? response ?? []);
    } catch (loadError: any) {
  ...
  return (
    <div className="page-shell py-10 sm:py-12">
      <div className="mb-6">
        ...
        <h1 className="mt-4 text-4xl font-bold text-stone-900">Manage delivery after payment clears</h1>
                ...
                <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
                  <div className="space-y-4">
                    ...
                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                      <div className="rounded-2xl bg-stone-50 px-4 py-3">
                        <p className="text-[11px] uppercase tracking-[0.16em] text-stone-400">Buyer</p>
                        <p className="mt-1 font-semibold text-stone-900">{order.buyerSnapshot?.fullName || "Buyer"}</p>
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-4 py-3">
                        <p className="text-[11px] uppercase tracking-[0.16em] text-stone-400">County</p>
                        <p className="mt-1 font-semibold text-stone-900">{order.delivery?.county || "Not set"}</p>
                      </div>
                      <div className="rounded-2xl bg-stone-50 px-4 py-3">
                        <p className="text-[11px] uppercase tracking-[0.16em] text-stone-400">Seller subtotal</p>
                        <p className="mt-1 font-semibold text-stone-900">{formatKes(order.sellerSubtotal) || "KES 0"}</p>
                      </div>
                    </div>
                      ...
                      {(order.items || []).map((item: any) => (
                        <div key={`${order._id}-${item.listingId}`} className="rounded-2xl border border-stone-200 bg-stone-50 px-4 py-4">
                          ...
                          <p className="mt-1 text-sm text-stone-600">
                            {item.quantity} {item.unit || "units"} | {formatKes(item.lineTotal) || "KES 0"}
                          </p>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="w-full max-w-sm space-y-4">
                    <div className="soft-panel p-4 text-sm text-stone-700">
                      ...
                      <p className="mt-2">
                        {formatLongDate(order.delivery?.estimatedDeliveryDate) || "Delivery date pending"}
                      </p>
                    </div>
