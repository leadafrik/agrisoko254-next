...
export default function ListingSellerPanel({ seller, listingId, sellerId }: Props) {
  ...
  const initFollow = async () => {
    ...
    setFollowInit(true);
    try {
      if (isAuthenticated && user?._id && !isOwnProfile) {
  ...
  const handleReport = async (e: React.FormEvent) => {
    ...
    setReporting(true);
    try {
      await submitReport(resolvedSellerId, reportReason, reportDesc, listingId);
      setReportDone(true);
    } catch { /* ignore */ } finally {
      setReporting(false);
    }
    ...
    <div className="surface-card p-6">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          {!verified && <p className="text-xs text-stone-500">Marketplace seller</p>}
        </div>
      </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          <div className="flex items-center gap-0.5">
            {[1, 2, 3, 4, 5].map((s) => (
              <Star
                key={s}
                className={`h-4 w-4 ${s <= stars ? "fill-amber-400 text-amber-400" : s - 0.5 <= stars ? "fill-amber-200 text-amber-400" : "text-stone-200"}`}
              />
            ))}
          </div>
          <span className="text-sm font-semibold text-stone-900">{ratingAvg.toFixed(1)}</span>
          <span className="text-xs text-stone-500">({ratingCount} review{ratingCount === 1 ? "" : "s"})</span>
        </div>
      )}

      {/* Follower count + response time + last active */}
      <div className="mt-4 space-y-2 text-xs text-stone-500">
        ...
        {lastActive && (
          <p className="flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full bg-forest-400 inline-block" />
            {lastActive}
          </p>
        )}
      </div>
      ...
      <div className="mt-5 space-y-2">
        ...
        {isAuthenticated && !isOwnProfile && resolvedSellerId && (
          <Link
            href={`/messages?seller=${resolvedSellerId}`}
        ...
        {!isAuthenticated && resolvedSellerId && (
          <Link
            href={`/login?redirect=${encodeURIComponent(`/messages?seller=${resolvedSellerId}`)}`}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-terra-500 py-2.5 text-sm font-semibold text-white transition hover:bg-terra-600"
          >
        ...
        {isAuthenticated && !isOwnProfile && resolvedSellerId && (
          ...
          >
            {isFollowing ? (
              <><UserCheck className="h-4 w-4 text-forest-600" /> Following</>
            ) : (
              <><UserPlus className="h-4 w-4" /> Follow seller</>
            )}
          </button>
        )}
      </div>
        ...
        <div className="mt-4 border-t border-stone-100 pt-4">
          {!showReport ? (
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-stone-400 hover:text-red-600 transition"
            >
              <Flag className="h-3.5 w-3.5" /> Report seller
            </button>
          ) : reportDone ? (
            <p className="text-xs text-forest-700">Report submitted. Thank you.</p>
          ) : (
            <form onSubmit={handleReport} className="space-y-2">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setShowReport(false)} className="flex-1 rounded-xl border border-stone-200 py-1.5 text-xs font-semibold text-stone-600">
                  Cancel
                </button>
                <button type="submit" disabled={reporting} className="flex-1 rounded-xl bg-red-600 py-1.5 text-xs font-semibold text-white hover:bg-red-700 disabled:opacity-50">
